...
pub unsafe extern "C" fn godotjs_free_transpile_result(p: *mut TranspileResult) {
    ...
    if !r.code.is_null() {
        unsafe { drop(Vec::from_raw_parts(r.code, r.code_len, r.code_len)) };
    }
    if !r.sourcemap.is_null() {
        unsafe { drop(Vec::from_raw_parts(r.sourcemap, r.sourcemap_len, r.sourcemap_len)) };
    }
    if !r.error.is_null() {
        unsafe { drop(Vec::from_raw_parts(r.error, r.error_len, r.error_len)) };
    }
}
