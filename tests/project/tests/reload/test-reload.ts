...
async function runReloadTest(): Promise<void> {
    ...
    try {
        ...
        if (!writer) throw new Error("failed to open target.ts for write");
        ...
        console.log(`TestReload: .ts hot reload OK (${elapsed}ms)`);
    } finally {
